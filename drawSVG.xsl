...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="sudoku">
        <svg width="500" height="700">
            ...
            <xsl:for-each select="region">
                ...
                <xsl:for-each select="row">
                   ...
                   <!-- question 3.b, on n'a pas réussi mais voici certains essaie que nous avons tester  -->
         <!-- premier test -->
        ...
        <!-- <xsl:if test="$valid-row and $valid-col and $valid-reg">
            <xsl:text>La grille est correct</xsl:text>
        </xsl:if>
        <xsl:if test="not($valid-row or $valid-col or $valid-reg)">
            <xsl:text>La grille n'est pas correct/xsl:text>
        </xsl:if> -->
